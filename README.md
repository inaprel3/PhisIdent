...
As a result of this work, the intelligent system PhisIdent for detecting phishing messages and URLs was developed and tested, combining machine learning methods, natural language processing, and web resource analysis. The development featured a modular architecture with a Python backend, an HTML, CSS, and JS frontend, and model storage in PKL and CSV formats, ensuring flexibility, scalability, and ease of integration into real-world environments. Testing demonstrated high system effectiveness, with most models achieving classification accuracy of 90–97%, confirming the appropriateness of the chosen algorithms (NB, LR, SVM, RF, and GB) and the representativeness of the datasets used. A key factor in improving system accuracy and adaptability is the user feedback mechanism, which allows refining classification results and gradually enhancing the models. Thus, PhisIdent serves as a practical tool for improving cybersecurity for organizations, educational institutions, and individual users, while future development could include integration with phishing domain databases, image analysis, and automatic model updates, further increasing effectiveness in detecting modern threats.
